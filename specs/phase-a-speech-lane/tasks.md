...
**Remaining (need a live streaming runner / data download / cloud key):** Tasks 1 (LibriSpeech+noise set), 3/4 (live streaming runner that produces state-seqs + real TTFA + drives the scorers), 5 (vendor adapter), 6 (live text-vs-lane probe), 10 (full run + DECISION.md), 11 (spec-verifier).
