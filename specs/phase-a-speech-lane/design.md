...
- **D8 — Score self-correction via structured tool-call grounding, NOT post-hoc LLM-judge extraction** (serves AC-6; from the focused arxiv run, run `wf_42258104-b4d`). SOTA voice benchmarks grade intent by *verifiable outcome*: τ-Voice — "success is deterministically evaluated by comparing the end state of the environment vs a gold standard"; FDB-v3 Pass@1 = agent invokes the expected tool with correct args [paper, 3-0]. So the runner constrains the agent to emit a structured action and grades its **arguments** against `corrected_intent`. **Argument matching uses SOTA tolerance** (FDB-v3's GPT-4o judge accepts ±5% numeric, case/format variation, aliases) — implemented in `conversational_scoring.match_value` — because the dominant speech failure is *argument-VALUE* (54–57%), not format. **Critical refinement:** the ±5% tolerance is a *magnitude* concept and is suppressed for identifier keys (order_id/code/phone…) via `_numeric_tol_for_key` — else `order_id 456` would "match" `459`. **LLM-judge (option a) is rejected as ground truth** (κ≈0.43 human agreement; a 3-judge ensemble rejected 19/25 human-accepted traces; judges are *more* brittle to formatting than meaning [paper, 2604.16706 / 2603.05399]) — usable only as a triage fallback for free-form turns, never as the pass/fail boundary. Rejected: (a) as primary, (c) heuristic parse (not SOTA).
